"""Run one locked GraTA dynamic-learning-rate sweep combination."""
...
def resolve_learning_rate(value: str) -> tuple[float, str]:
    try:
        ...
        raise ValueError(f"Invalid GraTA learning-rate ceiling: {value!r}") from error
    try:
        tag = SWEEP_LEARNING_RATES[decimal_value]
    except KeyError as error:
        allowed = ", ".join(SWEEP_LEARNING_RATES.values())
        raise ValueError(
            f"GraTA sweep learning-rate ceiling must be one of: {allowed}"
        ) from error
...
def configure_sweep(
    ...
    resolved["methods"]["grata"]["lr"] = learning_rate
    resolved["methods"]["grata"]["profile_kind"] = "lr_sweep"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--results-root",
        default="results/Stochastic_Ini_ForegroundOnly/grata_lr_sweep",
    )
...
def main() -> None:
    ...
    print(json.dumps({
        ...
        "learning_rate_ceiling": learning_rate,
