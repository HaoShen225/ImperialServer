...
from helper.DSU import DistributionUncertainty
...
DEFAULT_RESULT_ROOT = PROJECT_ROOT / "TrainingData" / "DSU"
METHOD = "DSU"
FIXED_SADG_RHO = 0.05
...
def reset_or_prepare_result_root(result_root: Path, overwrite: bool) -> Path:
    ...
    if result_root.exists() and overwrite:
        ...
        if any((result_root / name).exists() for name in expected):
            raise FileExistsError(f"Result root already contains DSU outputs. Use --overwrite: {result_root}")
...
class DSUSegmentationModel(nn.Module):
    """Insert one DSU layer into the existing helper U-Net backbone."""

    def __init__(self, backbone: nn.Module, dsu: DistributionUncertainty, position: str = "enc1"):
        super().__init__()
        if str(position) not in {"enc1", "enc2", "enc3", "bottleneck"}:
            raise ValueError(f"Unsupported DSU position: {position!r}")
        if not hasattr(backbone, "base"):
            raise TypeError("DSUSegmentationModel expects the helper Dec1L2NormUNet-style wrapper.")
    ...
    def _dsu_if_needed(self, name: str, feature: torch.Tensor) -> torch.Tensor:
        if self.position == name:
            return self.dsu(feature)
        return feature
    ...
    def forward(self, x: torch.Tensor, return_features: bool = False):
        ...
        enc1 = self._dsu_if_needed("enc1", m.enc1(x))
        enc2 = self._dsu_if_needed("enc2", m.enc2(m.pool1(enc1)))
        enc3 = self._dsu_if_needed("enc3", m.enc3(m.pool2(enc2)))
        bottleneck = self._dsu_if_needed("bottleneck", m.bottleneck(m.pool3(enc3)))
...
def build_dsu_model(args: argparse.Namespace, device: torch.device) -> DSUSegmentationModel:
    backbone = build_model(model_config(args))
    dsu = DistributionUncertainty(
        p=float(args.dsu_p),
        eps=float(args.dsu_eps),
        factor=float(args.dsu_factor),
    )
    return DSUSegmentationModel(backbone, dsu, position=str(args.dsu_position)).to(device)
...
def dsu_metadata(args: argparse.Namespace) -> Dict[str, Any]:
    return {
        "method": METHOD,
        "sota_method": METHOD,
        "dsu_p": float(args.dsu_p),
        "dsu_eps": float(args.dsu_eps),
        "dsu_factor": float(args.dsu_factor),
        "dsu_position": str(args.dsu_position),
...
def train_one_run(
    *,
    ...
    model = build_dsu_model(args, device)
    ...
    print(
        ...
        f"dsu={args.dsu_position}/p{float(args.dsu_p):.3g}/factor{float(args.dsu_factor):.3g} "
        ...
        item.update(
            {
                "method": METHOD,
                "sota_method": METHOD,
                "dsu_p": finite_mean(row.get("dsu_p") for row in rows),
                "dsu_factor": finite_mean(row.get("dsu_factor") for row in rows),
        ...
        out.append(
            {
                ...
                "dsu_p": finite_mean(row.get("dsu_p") for row in rows),
                "dsu_factor": finite_mean(row.get("dsu_factor") for row in rows),
                "dsu_position": rows[0].get("dsu_position", ""),
                "sadg_rho": finite_mean(row.get("sadg_rho") for row in rows),
                "lambda_disp": finite_mean(row.get("lambda_disp") for row in rows),
                "dice_weight": finite_mean(row.get("dice_weight") for row in rows),
...
def write_analysis_report(result_root: Path, overall_rows: Sequence[Mapping[str, Any]]) -> None:
    lines = [
        ...
        "DSU is active only during training and disabled automatically during evaluation.",
...
def run(args: argparse.Namespace) -> None:
    ...
    print(
        f"[METHOD] {METHOD} position={args.dsu_position} p={float(args.dsu_p):.6g} "
        f"factor={float(args.dsu_factor):.6g} sadg_rho={float(args.sadg_rho):.6g} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train DSU with SADG(CE+0.5Dice)+0.05Dispersion.")
    ...
    parser.add_argument("--dsu_p", type=float, default=0.5)
    parser.add_argument("--dsu_eps", type=float, default=1e-6)
    parser.add_argument("--dsu_factor", type=float, default=1.0)
    parser.add_argument("--dsu_position", default="enc1", choices=("enc1", "enc2", "enc3", "bottleneck"))
