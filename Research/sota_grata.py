#!/usr/bin/env python3
"""Run GraTA on the 15-backbone ACDC -> MMS experiment matrix."""
...
from helper.Grata import GraTa, collect_params, configure_model
...
DEFAULT_OUTPUT = RESEARCH_ROOT / "grata_results"
PATIENT_SETTINGS = (1, 2, 3)
SEEDS = (0, 1, 2, 3, 4)
VENDORS = ("A", "B", "C", "D")
SUMMARY_FIELDS = (
    "method", "task_id", "patient_setting", "seed", "vendor", "vendor_name", "domain",
    "checkpoint", "batch_size", "optimizer", "base_learning_rate", "steps", "episodic",
    "stream_order", "adapted_parameter_names", "n_patients", "n_slices", "dice_rv",
    "dice_myo", "dice_lv", "dice_mean", "hd95_rv", "hd95_myo", "hd95_lv", "hd95_mean",
    "mean_aux_loss", "mean_pseudo_loss", "mean_gradient_cosine", "mean_dynamic_learning_rate",
    "output_dir",
)
TRACE_FIELDS = (
    "task_id", "patient_id", "phase", "z_index", "slice_id", "batch_index",
    "aux_entropy_loss", "consistency_loss", "gradient_cosine", "dynamic_learning_rate",
)
...
def validate_inputs(backbone_root: Path, dataset_root: Path) -> None:
    ...
    if len(discovered) != 15 or missing or unexpected:
        raise RuntimeError(
            f"Expected exactly 15 backbones; found={len(discovered)} missing={missing} unexpected={unexpected}"
        )
...
def load_backbone(path: Path, patient_setting: int, seed: int, device: torch.device) -> tuple[UNet, Dict[str, Any]]:
...
def run_task(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    optimizer = torch.optim.Adam(params, lr=float(args.learning_rate), betas=(0.9, 0.999), weight_decay=0.0)
    grata = GraTa(
        model, optimizer, base_lr=float(args.learning_rate), steps=int(args.steps), episodic=bool(args.episodic)
    )
    backbone_id = f"MeanTeacher_Patient{patient_setting}_Seed{seed}_teacher_GraTA"
            ...
            trace_rows.append(
                {
                    ...
                    "aux_entropy_loss": grata.last_aux_loss,
                    "consistency_loss": grata.last_pseudo_loss,
                    "gradient_cosine": grata.last_cosine,
                    "dynamic_learning_rate": grata.last_learning_rate,
                }
            )
        evaluator.update(
    ...
    summary: Dict[str, Any] = {
        ...
        "mean_aux_loss": finite_mean([row["aux_entropy_loss"] for row in trace_rows]),
        "mean_pseudo_loss": finite_mean([row["consistency_loss"] for row in trace_rows]),
        "mean_gradient_cosine": finite_mean([row["gradient_cosine"] for row in trace_rows]),
        "mean_dynamic_learning_rate": finite_mean([row["dynamic_learning_rate"] for row in trace_rows]),
    ...
    print(f"[COMPLETE] task={task_id} dice={summary['dice_mean']:.6f} hd95={summary['hd95_mean']:.6f}", flush=True)
    return summary
...
def aggregate(output_root: Path) -> None:
    ...
    if len(rows) != 60:
        raise RuntimeError(f"GraTA matrix incomplete: {len(rows)}/60 tasks")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run or aggregate ACDC -> MMS GraTA tasks.")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.learning_rate <= 0 or args.steps <= 0 or args.max_patients < 0:
        raise ValueError("learning-rate/steps must be positive and max-patients non-negative.")
