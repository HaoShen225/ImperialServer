...
DEFAULT_RESULT_ROOT = PROJECT_ROOT / "TrainingData" / "ProtoInTEnt_RadialGateTTA"
...
RADIAL_FUSE_UNIFORM = "RadialGate-fuse-uniform"
RADIAL_FUSE_PREDENTROPY = "RadialGate-fuse-predentropy"
RADIAL_FUSE_PROTOENTROPY = "RadialGate-fuse-protoentropy"
DIAGNOSTIC_VARIANTS = (
    SADG_NONE,
    RADIAL_NONE,
    RADIAL_LCC,
...
TTA_VARIANTS = (
    ...
    RADIAL_NONE,
    RADIAL_FUSE_UNIFORM,
    RADIAL_FUSE_PREDENTROPY,
    RADIAL_FUSE_PROTOENTROPY,
)
DEFAULT_TTA_VARIANTS = TTA_VARIANTS
DIAGNOSTIC_VARIANT_ALIASES = {
    ...
    "uniform": RADIAL_FUSE_UNIFORM,
    "fuse-uniform": RADIAL_FUSE_UNIFORM,
    "radialgate-fuse-uniform": RADIAL_FUSE_UNIFORM,
    "predentropy": RADIAL_FUSE_PREDENTROPY,
    "fuse-predentropy": RADIAL_FUSE_PREDENTROPY,
    "radialgate-fuse-predentropy": RADIAL_FUSE_PREDENTROPY,
    "protoentropy": RADIAL_FUSE_PROTOENTROPY,
    "fuse-protoentropy": RADIAL_FUSE_PROTOENTROPY,
    "radialgate-fuse-protoentropy": RADIAL_FUSE_PROTOENTROPY,
}
...
def normalize_tta_variant(name: str) -> str:
    key = str(name).strip()
    lowered = key.lower()
    if lowered in DIAGNOSTIC_VARIANT_ALIASES:
        variant = DIAGNOSTIC_VARIANT_ALIASES[lowered]
        if variant in TTA_VARIANTS:
            return variant
    for variant in TTA_VARIANTS:
        if lowered == variant.lower():
            return variant
    raise ValueError(f"Unknown TTA variant {name!r}; expected one of {', '.join(TTA_VARIANTS)}")


def parse_tta_variants(text: str) -> List[str]:
    variants = [normalize_tta_variant(name) for name in parse_str_list(text)]
    if not variants:
        raise ValueError("At least one TTA variant must be provided.")
    seen: set[str] = set()
    out: List[str] = []
    for variant in variants:
        if variant not in seen:
...
def prompt_args_from_checkpoint(args: argparse.Namespace, metadata: Mapping[str, Any]) -> argparse.Namespace:
    ...
    prompt_args.prompt_root = str(resolve_path(args.prototype_root))
...
def prototype_path_for_run(args: argparse.Namespace, metadata: Mapping[str, Any], shot: int, seed: int) -> Path:
    prompt_args = prompt_args_from_checkpoint(args, metadata)
    return (
        prompt_run_dir(prompt_args, SEG_SADG_ABLATION, LF_BARYCENTER_RADIAL_GATE, int(shot), int(seed))
        / str(args.prototype_file_name)
    )


def load_source_prototype_bank(
    args: argparse.Namespace,
    *,
    ...
    seed: int,
    device: torch.device,
) -> tuple[Dict[str, Dict[str, Dict[int, torch.Tensor]]], Dict[str, Any], Path]:
    path = prototype_path_for_run(args, metadata, int(shot), int(seed))
    if not path.exists():
        raise FileNotFoundError(f"Missing source prototype bank: {path}")
    payload = safe_torch_load(path, map_location="cpu")
    if int(payload.get("schema_version", 0)) != 2:
        raise ValueError(f"Prototype bank {path} must have schema_version=2.")
    if payload.get("prototype_kind") != "source_multilayer_gt_kmeans":
        raise ValueError(f"Prototype bank {path} has unexpected kind {payload.get('prototype_kind')!r}.")
    layers = parse_str_list(args.prototype_layers)
    bank: Dict[str, Dict[str, Dict[int, torch.Tensor]]] = {}
    for gtag, gamma_payload in payload["prototypes"].items():
        bank[gtag] = {}
        for layer_name in layers:
            if layer_name not in gamma_payload:
                raise KeyError(f"Prototype bank {path} does not contain layer {layer_name!r} for {gtag}.")
            class_payload = gamma_payload[layer_name]["class_prototypes"]
            bank[gtag][layer_name] = {
                int(cls): tensor.detach().float().to(device)
                for cls, tensor in class_payload.items()
            }
    return bank, payload, path
...
def softmax_weight_from_entropy(entropies: Sequence[float], temperature: float) -> List[float]:
    vals = torch.tensor([float(x) for x in entropies], dtype=torch.float32)
    vals = torch.nan_to_num(vals, nan=float("inf"), posinf=float("inf"), neginf=0.0)
    finite = torch.isfinite(vals)
    if not bool(finite.any().item()):
        return [1.0 / float(len(entropies)) for _ in entropies]
    max_finite = vals[finite].max()
    vals = torch.where(finite, vals, max_finite)
    weights = torch.softmax(-vals / max(float(temperature), 1e-8), dim=0)
    return [float(x) for x in weights.tolist()]


def one_hot_gamma_weights(gamma_values: Sequence[float], target_gamma: float = 1.0) -> List[float]:
    idx = int(np.argmin([abs(float(gamma) - float(target_gamma)) for gamma in gamma_values]))
    return [1.0 if i == idx else 0.0 for i in range(len(gamma_values))]


def format_metric_list(values: Sequence[float]) -> str:
    return "|".join(f"{float(x):.8g}" for x in values)


def format_layer_entropy(layer_entropy: Mapping[str, Sequence[float]], layer_name: str) -> str:
    return format_metric_list(layer_entropy.get(layer_name, []))


def normalized_prediction_entropy(probs: torch.Tensor) -> torch.Tensor:
    n_classes = int(probs.shape[1])
    entropy = -(probs.clamp_min(1e-8) * probs.clamp_min(1e-8).log()).sum(dim=1)
    return entropy / max(float(np.log(float(n_classes))), 1e-8)


def prototype_entropy_map(
    feature: torch.Tensor,
    class_prototypes: Mapping[int, torch.Tensor],
    *,
    proto_temperature: float,
) -> torch.Tensor:
    z = F.normalize(feature.float(), p=2, dim=1, eps=1e-8)
    b, d, h, w = z.shape
    flat = z.permute(0, 2, 3, 1).reshape(-1, d)
    scores: List[torch.Tensor] = []
    for cls in sorted(class_prototypes):
        protos = F.normalize(class_prototypes[int(cls)].to(device=flat.device, dtype=flat.dtype), p=2, dim=1, eps=1e-8)
        sim = flat @ protos.t()
        scores.append(sim.max(dim=1).values.reshape(b, h, w))
    score_tensor = torch.stack(scores, dim=1)
    q = torch.softmax(score_tensor / max(float(proto_temperature), 1e-8), dim=1)
    entropy = -(q.clamp_min(1e-8) * q.clamp_min(1e-8).log()).sum(dim=1)
    return entropy / max(float(np.log(float(len(scores)))), 1e-8)


def resize_region_mask(region: torch.Tensor, size: Sequence[int]) -> torch.Tensor:
    if tuple(region.shape[-2:]) == tuple(size):
        return region
    return F.interpolate(region[:, None].float(), size=tuple(size), mode="nearest")[:, 0] > 0.5


def accumulate_entropy(values: torch.Tensor, region: torch.Tensor) -> tuple[float, int]:
...
def predict_tta_domain_cases(
    ...
    *,
    ...
    prototype_bank: Mapping[str, Mapping[str, Mapping[int, torch.Tensor]]],
    prototype_payload: Mapping[str, Any],
    prototype_path: Path,
    shot: int,
    seed: int,
    ...
    gamma_tags = [gamma_tag(x) for x in gamma_values]
    missing = [gtag for gtag in gamma_tags if gtag not in prototype_bank]
    if missing:
        raise KeyError(f"Prototype bank {prototype_path} is missing gamma entries: {missing}")

    cases_by_variant: Dict[str, List[Dict[str, Any]]] = {variant: [] for variant in variants}
    ...
    for case_id in dataset.grouped_case_ids():
        ...
        images = torch.stack([torch.from_numpy(item["image"])[None, ...].float() for item in items], dim=0)
        gt_stack = np.stack([item["mask"] for item in items], axis=0).astype(np.int64)
        prob_chunks: Dict[str, List[torch.Tensor]] = {gtag: [] for gtag in gamma_tags}
        pred_entropy_sum = {gtag: 0.0 for gtag in gamma_tags}
        pred_entropy_count = {gtag: 0 for gtag in gamma_tags}
        proto_entropy_sum = {gtag: 0.0 for gtag in gamma_tags}
        proto_entropy_count = {gtag: 0 for gtag in gamma_tags}
        proto_layer_sum = {gtag: {layer: 0.0 for layer in layers} for gtag in gamma_tags}
        proto_layer_count = {gtag: {layer: 0 for layer in layers} for gtag in gamma_tags}

        for start in range(0, images.shape[0], int(args.eval_batch_size)):
            batch = images[start:start + int(args.eval_batch_size)].to(device)
            batch_probs: Dict[str, torch.Tensor] = {}
            batch_features: Dict[str, Mapping[str, torch.Tensor]] = {}
            for gamma, gtag in zip(gamma_values, gamma_tags):
                ...
                if not isinstance(out, Mapping) or "features" not in out:
                    raise RuntimeError("Model did not return feature dictionary with return_features=True.")
                probs = torch.softmax(out["logits"], dim=1)
                batch_probs[gtag] = probs
                batch_features[gtag] = out["features"]
                prob_chunks[gtag].append(probs.detach().cpu())

            fg_union = torch.zeros(batch.shape[0], batch.shape[-2], batch.shape[-1], dtype=torch.bool, device=device)
            for probs in batch_probs.values():
                fg_union |= probs[:, 1:, :, :].amax(dim=1) > float(args.foreground_threshold)
            if int(fg_union.sum().item()) <= 0:
                fg_union = torch.ones_like(fg_union, dtype=torch.bool)

            for gtag in gamma_tags:
                pred_entropy = normalized_prediction_entropy(batch_probs[gtag])
                val_sum, val_count = accumulate_entropy(pred_entropy, fg_union)
                pred_entropy_sum[gtag] += val_sum
                pred_entropy_count[gtag] += val_count
                layer_entropies: List[torch.Tensor] = []
                for layer_name in layers:
                    feature = batch_features[gtag][layer_name]
                    layer_region = resize_region_mask(fg_union, feature.shape[-2:])
                    entropy_map = prototype_entropy_map(
                        feature,
                        prototype_bank[gtag][layer_name],
                        proto_temperature=float(args.proto_temperature),
                    )
                    layer_sum, layer_count = accumulate_entropy(entropy_map, layer_region)
                    proto_layer_sum[gtag][layer_name] += layer_sum
                    proto_layer_count[gtag][layer_name] += layer_count
                    layer_entropies.append(entropy_map)

        probs_by_gamma = {gtag: torch.cat(prob_chunks[gtag], dim=0).float() for gtag in gamma_tags}
        pred_entropy_values = [
            pred_entropy_sum[gtag] / max(int(pred_entropy_count[gtag]), 1)
            for gtag in gamma_tags
        ]
        proto_layer_values: Dict[str, List[float]] = {}
        for layer_name in layers:
            proto_layer_values[layer_name] = [
                proto_layer_sum[gtag][layer_name] / max(int(proto_layer_count[gtag][layer_name]), 1)
                for gtag in gamma_tags
            ]
        proto_entropy_values = [
            float(np.mean([proto_layer_values[layer][i] for layer in layers]))
            for i in range(len(gamma_values))
        ]

        weights_by_variant: Dict[str, List[float]] = {}
        if RADIAL_NONE in variants:
            weights_by_variant[RADIAL_NONE] = one_hot_gamma_weights(gamma_values, target_gamma=1.0)
        if RADIAL_FUSE_UNIFORM in variants:
            weights_by_variant[RADIAL_FUSE_UNIFORM] = [1.0 / float(len(gamma_values)) for _ in gamma_values]
        if RADIAL_FUSE_PREDENTROPY in variants:
            weights_by_variant[RADIAL_FUSE_PREDENTROPY] = softmax_weight_from_entropy(
                pred_entropy_values,
                float(args.fusion_weight_temperature),
            )
        if RADIAL_FUSE_PROTOENTROPY in variants:
            weights_by_variant[RADIAL_FUSE_PROTOENTROPY] = softmax_weight_from_entropy(
                proto_entropy_values,
                float(args.fusion_weight_temperature),
            )
        ...
        for variant in variants:
            ...
            fused = torch.zeros_like(next(iter(probs_by_gamma.values())))
            for gtag, weight in zip(gamma_tags, weights):
                fused += float(weight) * probs_by_gamma[gtag]
            pred_stack = torch.argmax(fused, dim=1).cpu().numpy().astype(np.int64)
            cases_by_variant[variant].append(
                {
                    "case_id": str(case_id),
                    "n_slices": int(len(items)),
                    "sagittal_x_indices": "|".join(str(int(item["sagittal_x_index"])) for item in items),
            ...
            branch_row: Dict[str, Any] = {
                ...
                "model_family": "RadialGate",
                "shot": int(shot),
                "seed": int(seed),
                "domain": dataset.domain_path.name,
                "case_id": str(case_id),
                "n_slices": int(len(items)),
                "gamma_values": format_metric_list(gamma_values),
                "gamma_weights": format_metric_list(weights),
                "gamma_weight_sum": float(sum(weights)),
                "pred_entropy_by_gamma": format_metric_list(pred_entropy_values),
                "proto_entropy_by_gamma": format_metric_list(proto_entropy_values),
                "prototype_path": str(prototype_path),
                "prototype_schema_version": int(prototype_payload.get("schema_version", 0)),
                "entropy_region": str(args.entropy_region),
            }
            for layer_name in layers:
                branch_row[f"proto_entropy_{layer_name}"] = format_layer_entropy(proto_layer_values, layer_name)
    ...
    return cases_by_variant, dataset_meta, branch_rows
...
def annotate_tta_rows(rows: Sequence[Dict[str, Any]], *, variant: str, gamma_values: Sequence[float], prototype_path: Path | None = None, prototype_payload: Mapping[str, Any] | None = None) -> None:
    for row in rows:
        row["tta_variant"] = variant
        row["gamma_values"] = format_metric_list(gamma_values)
        if prototype_path is not None:
            row["prototype_path"] = str(prototype_path)
        if prototype_payload is not None:
            row["prototype_schema_version"] = int(prototype_payload.get("schema_version", 0))
        row["entropy_region"] = "n/a" if variant == SADG_NONE else "foreground_union"


def paired_tta_delta_summary(eval_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    by_key = {
        (row.get("tta_variant"), int(row.get("shot", 0)), int(row.get("seed", 0)), row.get("domain", "")): row
        for row in eval_rows
    }
    comparisons = [
        ("RadialGate-none_vs_SADG-none", RADIAL_NONE, SADG_NONE),
        ("RadialGate-fuse-uniform_vs_RadialGate-none", RADIAL_FUSE_UNIFORM, RADIAL_NONE),
        ("RadialGate-fuse-predentropy_vs_RadialGate-none", RADIAL_FUSE_PREDENTROPY, RADIAL_NONE),
        ("RadialGate-fuse-protoentropy_vs_RadialGate-none", RADIAL_FUSE_PROTOENTROPY, RADIAL_NONE),
        ("RadialGate-fuse-protoentropy_vs_RadialGate-fuse-predentropy", RADIAL_FUSE_PROTOENTROPY, RADIAL_FUSE_PREDENTROPY),
    ]
    pair_rows: List[Dict[str, Any]] = []
    keys = sorted({(int(row.get("shot", 0)), int(row.get("seed", 0)), row.get("domain", "")) for row in eval_rows})
    for label, variant, base in comparisons:
...
def write_tta_report(result_root: Path, eval_rows: Sequence[Dict[str, Any]], overall_domain: Sequence[Dict[str, Any]], delta_rows: Sequence[Dict[str, Any]]) -> None:
    all_delta = [row for row in delta_rows if row.get("domain") == "ALL_DOMAINS"]
    lines = [
        "# Proto-InTEnt RadialGate TTA Report",
        "",
        f"- Domain-level rows: {len(eval_rows)}",
        f"- Variants: {', '.join(sorted({str(row.get('tta_variant')) for row in eval_rows}))}",
        "- Dice 越大越好；HD95 越小越好。",
        "",
        "## Overall 15-run Domain Summary",
        "",
...
def write_tta_outputs(
    ...
    *,
    ...
    ensure_dir(result_root)
    write_csv(result_root / "tta_eval_metrics.csv", eval_rows)
    write_csv(result_root / "tta_case_metrics.csv", case_rows)
    write_csv(result_root / "tta_domain_class_metrics.csv", class_rows)
    write_csv(result_root / "tta_branch_diagnostics.csv", branch_rows)
    domain_5seed = summarize_diagnostic_groups(eval_rows, ("tta_variant", "shot", "domain"))
    overall_domain = summarize_diagnostic_groups(eval_rows, ("tta_variant", "domain"))
    delta_rows = paired_tta_delta_summary(eval_rows)
    write_csv(result_root / "domain_5seed_summary.csv", domain_5seed)
    write_csv(result_root / "overall_15run_domain_summary.csv", overall_domain)
    write_csv(result_root / "hypothesis_delta_summary.csv", delta_rows)
    write_tta_report(result_root, eval_rows, overall_domain, delta_rows)


def run_tta(args: argparse.Namespace) -> None:
    device = resolve_device(args.device)
    result_root = ensure_dir(resolve_path(args.result_root))
    variants = parse_tta_variants(args.tta_variants)
    gamma_values = parse_float_list(args.tta_gamma_values)
    shots = parse_int_list(args.shots)
    seeds = parse_int_list(args.seeds)
    if not gamma_values:
        raise ValueError("At least one TTA gamma value is required.")
    if str(args.entropy_region) != "foreground_union":
        raise ValueError("Only --entropy_region foreground_union is implemented for this diagnostic script.")
    write_json(
        result_root / "run_config.json",
        {
            **vars(args),
            "tta_variants": variants,
            "tta_gamma_values": [float(x) for x in gamma_values],
            ...
            "prototype_root": str(resolve_path(args.prototype_root)),
        },
    )
    ...
    print(f"[TTA_VARIANTS] {variants}", flush=True)
    print(f"[GAMMAS] {gamma_values}", flush=True)
    ...
    for shot in shots:
        for seed in seeds:
            ...
            prototype_bank: Dict[str, Dict[str, Dict[int, torch.Tensor]]] | None = None
            ...
            if radial_variants:
                ...
                prototype_bank, prototype_payload, prototype_path = load_source_prototype_bank(
                    args,
                    metadata=radial_meta,
                    shot=int(shot),
                    seed=int(seed),
                    device=device,
                )
                print(f"[LOAD] RadialGate shot={shot} seed={seed}: {radial_checkpoint}", flush=True)
                print(f"[LOAD] Prototype bank: {prototype_path}", flush=True)
            ...
            for domain in DOMAIN_NAMES:
                if sadg_model is not None:
                    ...
                    annotate_tta_rows([eval_row], variant=SADG_NONE, gamma_values=gamma_values)
                    annotate_tta_rows(case_rows, variant=SADG_NONE, gamma_values=gamma_values)
                    annotate_tta_rows(class_rows, variant=SADG_NONE, gamma_values=gamma_values)
                ...
                if radial_model is not None and prototype_bank is not None and prototype_payload is not None and prototype_path is not None:
                    train_ids = [str(x) for x in radial_meta.get("train_case_ids", [])]
                    cases_by_variant, dataset_meta, branch_rows = predict_tta_domain_cases(
                        radial_model,
                        args=args,
                        device=device,
                        domain=domain,
                        train_case_ids=train_ids,
                        variants=radial_variants,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate Proto-InTEnt style inference-time fusion for RadialGate checkpoints."
    )
    ...
    parser.add_argument("--tta_variants", default=",".join(DEFAULT_TTA_VARIANTS))
    parser.add_argument("--tta_gamma_values", default="0,0.25,0.5,0.75,1.0")
    parser.add_argument("--prototype_root", default=str(DEFAULT_PROMPT_ROOT))
    parser.add_argument("--prototype_file_name", default="source_multilayer_prototypes.pt")
    ...
    parser.add_argument("--fusion_weight_temperature", type=float, default=0.1)
    parser.add_argument("--entropy_region", default="foreground_union", choices=("foreground_union",))
