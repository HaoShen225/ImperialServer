"""Run one locked TENT learning-rate sweep combination."""
...
def resolve_learning_rate(value: str) -> tuple[float, str]:
    try:
        ...
        raise ValueError(f"Invalid TENT learning rate: {value!r}") from error
    try:
        tag = SWEEP_LEARNING_RATES[decimal_value]
    except KeyError as error:
        allowed = ", ".join(SWEEP_LEARNING_RATES.values())
        raise ValueError(
            f"TENT sweep learning rate must be one of: {allowed}"
        ) from error
...
def configure_sweep(
    ...
    resolved["methods"]["tent"]["lr"] = learning_rate
    resolved["methods"]["tent"]["profile_kind"] = "lr_sweep"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--results-root",
        default="results/Stochastic_Ini_ForegroundOnly/tent_lr_sweep",
    )
