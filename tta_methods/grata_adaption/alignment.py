"""Gradient-alignment primitives isolated from the original GraTA profile."""
...
def gradient_cosine(
    ...
    if len(first_values) != len(second_values):
        raise ValueError("GraTA-Adaption gradient vectors must have the same length")
...
def perturb_parameters(
    ...
    if len(parameter_values) != len(gradient_values):
        raise ValueError("GraTA-Adaption parameters and gradients must have the same length")
...
def restore_parameters(
    ...
    if len(parameter_values) != len(original_values):
        raise ValueError("GraTA-Adaption parameters and saved values must have the same length")
